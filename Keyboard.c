...
********************************************************************
 ...
 Change History:
  ...
  2.1   02/26/2007   Updated for simplicity and to use common
                     coding style
  3.0   01/31/2018   Adapted for use as the HID portion of a USB 
                     keyboard emulator
 ...
 *******************************************************************/
#if defined(__18CXX)
#define MAX485Init()      LATB &= 0xAF; TRISB &= 0xAF; // cmheong 2018-01-25.
// Initializes RB4, RB6 as output to control MAX485 Tx/Rx direction
// 0xAF defaults to receive mode.
#define MAX485_RE LATBbits.LATB4 // Logic is inverted (0=enable receiver)
#define MAX485_DE LATBbits.LATB6 // TX enable. Must set LATB4 also.

// WhenPB6 and PB4 are changed separately, PB6 does not stay set, but
// falls to zero. Seems the Read-modify-write is seeing too much load there.
#define MAX485RxMode() LATB &= 0xAF; // MAX485_RE = 0;MAX485_DE = 0;mLED_3_Off();
#define MAX485TxMode() LATB |= 0x50; // MAX485_RE = 1;MAX485_DE = 1;mLED_3_On();
...
{
    #if defined(__18CXX) // cmheong __18CXX *is* defined
            unsigned char c;
        #if defined(__18F14K50)
            //ANSELHbits.ANS11 = 0;     // Make RB5 digital so USART can use pin for Rx
            ANSELH = 0; // cmheong This disables A8-A11 (enables RC6, RC7, RB4, RB5)
                        //          we use RB4 for MAX485 RE (receiver enable)
            ANSEL = 0x00; //cmheong Disables A4-A7 (enables RC0-RC3 for USB LEDs)
            #ifndef BAUDCON
        ...
        UART_TRISRx=1;                          // RX
        UART_TRISTx=0;                          // TX
        TXSTA = 0x24;           // TX enable BRGH=1 (high baudrate) cmheong
        RCSTA = 0x90;           // cmheong B7-serial port enable, B4-CREN,
                                // required to enable Rx
        SPBRG = 0x71;
        SPBRGH = 0x02;          // 0x0271 for 48MHz -> 19200 baud
        BAUDCON = 0x08;         // BRG16 = 1
                                // cmheong For 1.5Mbps fosc 48MHz use
                                // BRG16=1, BRGH=1 n=8 from 2010-03-23
                                // cmheong stopped here 18:52 28-Sep-2010
        c = RCREG;                              // read
    #endif
}
...
unsigned char rs485_char=0; /* 2018-02-01 */
...
void Keyboard(void)
{
	...
	static unsigned char lshift = 0; // 2021-03-08 cmheong
    ...
    {
        if(mDataRdyUSART()) // 2018-02-01 
        {
            rs485_char=getcUSART();
            lshift = 0; // 2021-03-08 assume no shift key unless proven otherwise
            switch(rs485_char)
            {
              case 8: // Backspace
                key= 0x2a; // 2021-03-08 changed from 15
              break;
              case 9: // Tab 2021-03-08
                key= 0x2b;
              break;
              case 13: // CR 
              case 10: // LF
                key=40;
              break;
              case 14: // Capslock is Ctrl-N
                key= 0x39;
              break;
              case 15: // Ctrl-Alt-Del is Ctrl-O
                lshift= 0x03;
                key= 0x4c;
              break;
              case 17: // DC1 is Ctrl-Q
                key= 0x3a; // Use as F1
              break;
              case 18: // DC2 is Ctrl-R
                key= 0x3b; // Use as F2
              break;
              case 19: // DC3 is Ctrl-S
                key= 0x3e; // Use as F5
              break;
              case 20: // DC4 is Ctrl-T
                key= 0x3f; // Use as F6
              break;
              case 21: // NAK is Ctrl-U 2021-03-09
                key= 0x42; // Use as F9
              break;
              case 22: // SYN is Ctrl-V 2021-03-09
                key= 0x43; // Use as F10
              break;
              case 23: // ETB is Ctrl-W 2021-03-09
                key= 0x50; // Use as Left Arrow
              break;
              case 24: // CAN is Ctrl-X 2021-03-09
                key= 0x51; // Use as Down Arrow
              break;
              case 25: // EM  is Ctrl-Y 2021-03-09
                key= 0x4f; // Use as Right Arrow
              break;
              case 26: // SUB is Ctrl-Z 2021-03-09
                key= 0x52; // Use as Up Arrow
              break;
              case 27: // Esc 2021-03-09
                lshift= 0x00; 
                key= 0x29;
              break;
              case 33: // '!' 2021-03-08
              ...
              case 89:
              case 90: // 'Z'
                lshift = 0; // 2021-03-08 unecessary; documentation only
                key= rs485_char - 61;
              break;
              case 123: // '{' 
                lshift= 0x02;
              ...
              case 94: // '^' 2021-03-08
                ...
                key= 0x23;
              break;
              case 95: // '_' 2021-03-08
                lshift= 0x02; 
                key= 0x2d;
              break;
              case 126: // '~' 
                lshift = 0x02; // 2021-03-08
              case 96: // '`'
                key= 0x35; // 29; // 2021-03-08 changed from 50
              break;
              case 97: // 'a'
              case 98:
              case 99:
              ...
              case 121:
              case 122: // 'z'
                lshift = 0x02; // 2021-03-07 keyboard powers up with capslock on
                key= rs485_char - 93; // 2021-03-08 changed from 95
              break;
              case 127: // Del
                lshift = 0x00; // 2021-03-07 
                key= 0x4c; // 2021-03-08 
              break;

              default:
                key=0x2c; // default to Space
              break;
            }

       	    //Load the HID buffer
            hid_report_in[0] = lshift; // 2021-03-08 cmheong
            hid_report_in[1] = 0;
            hid_report_in[2] = key;
            hid_report_in[3] = 0;
            hid_report_in[4] = 0;
    ...
    if(!HIDRxHandleBusy(lastOUTTransmission))
    {
		...
		BlinkStatusValid = FALSE;	//Stop toggling the LEDs, so you can see the Num lock LED state instead.	
...
{
	//Add SET_REPORT control transfer handler here to receive
	//LED data.  Here would also be a good place to turn on
	//or off the LEDs on the keyboard.
}
